...
import {
  Loader2, Trophy, AlertTriangle, RotateCcw, ClipboardList,
  HelpCircle, Target, CheckCircle2, Frown, ArrowLeft, ArrowRight, Award, Lock,
} from 'lucide-react'
...
type Screen = 'info' | 'taking' | 'result'
...
export default function CourseQuizTaker({
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      try {
        const res = await fetch('/api/submit-course-quiz', {
          method: 'POST',
  ...
  // ── Info screen ───────────────────────────────────────────
  if (screen === 'info') {
    const isRetake = allAttempts.length > 0 && !alreadyPassed
    return (
      <div className="card" style={{ padding: '40px', maxWidth: '520px', margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          ...
          <div style={{ fontFamily: 'var(--font-serif)', fontWeight: 600, fontSize: '22px', marginBottom: '8px' }}>
            {isRetake ? `Retake — Attempt #${allAttempts.length + 1}` : quiz.title}
          </div>
          {isRetake && latestAttempt && (
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '6px 14px',
              ...
              <RotateCcw size={12}/> Last: {Math.round(latestAttempt.score_percent)}% — Need {quiz.passing_percent}%
            </div>
          )}
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: '12px', marginBottom: '28px' }}>
          {[
            ...
            { Icon: Target, val: `${quiz.passing_percent}%`, label: 'Pass mark' },
          ].map(s => (
        ...
        <div style={{ textAlign: 'center', marginTop: '14px' }}>
          <a href={`/courses/${courseSlug}`} style={{ fontSize: '13px', color: 'var(--muted)', textDecoration: 'none' }}>← Back to course</a>
        </div>
      </div>
    )
  }
      ...
      <div>
        ...
        {allAttempts.length > 1 && (
          <div className="card" style={{ marginBottom: '20px', padding: '16px 20px' }}>
            <div className="stat-label" style={{ marginBottom: '12px' }}>Attempt history</div>
        ...
        <a href={`/courses/${courseSlug}`} className="btn btn-ghost" style={{ fontSize: '13px' }}>
          <ArrowLeft size={13}/> Back to course
        </a>
      </div>
    )
  ...
  return (
    <div>
      ...
      <div className="progress-track" style={{ height: '4px', marginBottom: '20px' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '24px' }}>
        {questions.map((q, i) => (
          <button key={i} onClick={() => setCurrentQ(i)} style={{
            ...
            background: currentQ === i ? 'var(--accent)' : answers[q.id] ? 'var(--green-dim)' : 'rgba(255,255,255,0.06)',
