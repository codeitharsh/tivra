...
// Same XSS-safe escaping as the programme-completion certificate route —
// required any time user-controlled text (full_name, course title) is
// interpolated into SVG.
...
export async function GET(
  ...
  const courseTitle = escapeXml(rawCourseTitle)

  // Same proven layout as the programme-completion certificate, with the
  // badge relabeled "COURSE COMPLETION" so the two are visually
  // distinct at a glance, and no "passing every phase assessment" line
  // since self-paced courses complete via required lessons, not exams.
...
<svg width="1200" height="850" viewBox="0 0 1200 850" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
  ...
  <rect x="470" y="240" width="260" height="38" rx="19" fill="#00d4ff" opacity="0.12"/>
  <rect x="470" y="240" width="260" height="38" rx="19" fill="none" stroke="#00d4ff" stroke-width="1" opacity="0.5"/>
  <text x="600" y="265" text-anchor="middle" font-family="Arial, sans-serif" font-weight="700"
    font-size="14" letter-spacing="2" fill="#00d4ff">COURSE COMPLETION</text>
  ...
  <text x="600" y="464" text-anchor="middle" font-family="Arial, sans-serif"
    font-size="16" fill="#ffffff" opacity="0.45" letter-spacing="1">has successfully completed the self-paced course</text>

  <text x="600" y="520" text-anchor="middle" font-family="Arial Black, sans-serif" font-weight="900"
    font-size="32" fill="none" stroke="url(#accentGrad)" stroke-width="0.5" letter-spacing="1">${courseTitle}</text>
  <text x="600" y="520" text-anchor="middle" font-family="Arial Black, sans-serif" font-weight="900"
    font-size="32" fill="#ffffff" letter-spacing="1">${courseTitle}</text>

  <text x="600" y="572" text-anchor="middle" font-family="Arial, sans-serif"
    font-size="16" fill="#ffffff" opacity="0.5" letter-spacing="1">completing every required lesson in the course</text>
  ...
  return new NextResponse(svg, {
    headers: {
      ...
      'Content-Disposition': `inline; filename="tivra-course-completion-${verifyCode}.svg"`,
