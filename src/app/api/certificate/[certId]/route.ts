...
// Escape text before interpolating into SVG/XML to prevent stored-XSS via
// a crafted full_name (or any other user-controlled field) at registration.
...
export async function GET(
  ...
  // Escape every user-influenced or otherwise dynamic string before it goes into SVG
  const name       = escapeXml(rawName)
  const phaseTitle = escapeXml(rawPhaseTitle)
  const certTitle  = escapeXml(rawCertTitle)
...
<svg width="1200" height="850" viewBox="0 0 1200 850" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
  ...
  <image href="${CERTIFICATE_LOGO_DATA_URI}" xlink:href="${CERTIFICATE_LOGO_DATA_URI}" x="552" y="68" width="96" height="96" filter="url(#glow)"/>
  ...
  <text x="600" y="510" text-anchor="middle" font-family="Arial Black, sans-serif" font-weight="900"
    font-size="30" fill="none" stroke="url(#accentGrad)" stroke-width="0.5" letter-spacing="1">${phaseTitle}</text>
  <text x="600" y="510" text-anchor="middle" font-family="Arial Black, sans-serif" font-weight="900"
    font-size="30" fill="#ffffff" letter-spacing="1">${phaseTitle}</text>
